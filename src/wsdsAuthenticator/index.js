...
exports.handler = function(event, context, callback) {
    ...
    // validate the incoming token by finding it in the db and checking if its validated
    // produce the principal user identifier using the token

    var principalId = event.authorizationToken;
    
    var params = {
      TableName : 'WebSocketDomainSecurity',
      Key: {
        connectionId: event.authorizationToken
      }
    };
    ...
    ddb.get(params, function(err, data) {
      if (err) {
        ...
        if (!(data!=undefined && 
              data.Item!=undefined &&
              data.Item.validated!=undefined &&
              data.Item.validated === 'yes')){
          // here access is denied because the item is not validated or doesn't exist, 
          // the client will receive a 403 Access Denied response
          callback("Unauthorized", null);
        } else {
          // here the token is valid, a policy must be generated which will allow or deny access to the client
          // access is allowed, API Gateway will proceed with the backend integration configured on the method that was called
